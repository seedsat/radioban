...
class Oauth extends CI_Controller
{
    ...
    public function __construct(){
        ...
        define('CONSUMER_KEY', 'VpjgyeZZtv0iJjvsh5ar3ahHz');
        define('CONSUMER_SECRET', '6wBsSnJ2Xmajp6XH1dC5mCSDGLrm7pPsA28ZdASBWbFsCxdR5f');
        /* callbachURLをtwitter developで指定する */
        define('OAUTH_CALLBACK', 'https://www.radio-bulletin-board.com/oauth/twitter_callback');
    }
    ...
    public function twitter_callback()
    {
        ...
        /* 
           twitternameやscreen_nameが変わっていたらデータをアップデートさせる必要がある
           となると、検索する時にid_strのみではなく、nameやscreen_nameを合わせて検索する必要があるかも
        */

        /* 
            とりあえずサーチを書いてデータがなければインサート
            そして、ログイン情報を取得してhomeにリダイレクト
            ただ、通常の登録はメールで認証番号を送っているのでそこをどうするか？
        */
        ...
        $twitter_user_data = array(
            ...
            'introduction' => $user_info->description,
        );

        /* DBにデータが登録されているか検索 */
        $userdata = $this->users_model->serch_twitter_user($twitter_user_data);

        if(empty($userdata))
        {
            /* データがなければインサートしてログインしてhomeにリダイレクト */
            $this->users_model->insert_twitter_user($twitter_user_data);
            $this->login_model->twitter_userlogin($twitter_user_data['id_str']);
        }
        elseif(!empty($userdata) && $twitter_user_data['name'] != $userdata[0]['username'] || $twitter_user_data['screen_name'] != $userdata[0]['screen_name'] )
        {
            /* twitterの名前とアカウント名が違えばアップデートしてhomeにリダイレクト */
            $this->users_model->update_twitter_user($twitter_user_data);
            $this->login_model->twitter_userlogin($userdata[0]['id_str']);
        }
        else
        {
